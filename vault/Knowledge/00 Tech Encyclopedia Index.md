---
title: 00 Tech Encyclopedia Index
type: knowledge-index
entry_type: index
schema_version: tech-encyclopedia/v2
status: navigation
domain: Technology
created: 2026-08-24
updated: 2026-09-13
aliases:
  - Tech Encyclopedia
  - 기술 백과사전
tags:
  - Index
  - Technology
---

# 00 Tech Encyclopedia Index

> [!abstract] 기술 백과사전
> 이 문서는 “무엇이 있는가”를 찾는 색인입니다. 각 링크는 한 파일에서 한 개념만 설명합니다. 개념 사이의 흐름은 [[Knowledge Maps/AI Technology Knowledge Map|AI Technology Knowledge Map]]에서 봅니다.

## 이 문서의 역할

`Tech Knowledge/Knowledge/`의 canonical 개념 이름, 분야, 읽기 시작점을 제공합니다. `and`, `와/과`로 여러 개념을 묶은 기존 경로는 호환 인덱스로만 유지하며 브리핑의 깊이 읽기 링크로 사용하지 않습니다.

## 포함하는 개념

### AI 기초와 연결

- [[Knowledge/AI Systems/AI Agents|AI Agents]]
- [[Knowledge/AI Systems/Model Context Protocol|Model Context Protocol]]
- [[Knowledge/AI Systems/Retrieval-Augmented Generation|Retrieval-Augmented Generation]]
- [[Knowledge/AI Systems/Conversational Voice AI|Conversational Voice AI]]
- [[Knowledge/AI Systems/Vision-Language-Action Models|Vision-Language-Action Models]]
- [[Knowledge/AI Systems/Time-Series Foundation Models|Time-Series Foundation Models]]

- [[Knowledge/AI Systems/Zero-Shot Inference|Zero-Shot Inference]]

### 운영·평가·인프라

- [[Knowledge/Data Systems/Aggregate Metrics|Aggregate Metrics]]

- [[Knowledge/AI Systems/AI Inference Infrastructure|AI Inference Infrastructure]]
- [[Knowledge/AI Systems/Enterprise AI Operating Model|Enterprise AI Operating Model]]
- [[Knowledge/AI Systems/Agent Evaluation|Agent Evaluation]]
- [[Knowledge/AI Systems/Agent Observability|Agent Observability]]

- [[Knowledge/AI Systems/KV Cache|KV Cache]]
- [[Knowledge/Data Systems/Latency Percentiles|Latency Percentiles]]

### 보안·거버넌스

- [[Knowledge/Security/Zero-Knowledge Proofs|Zero-Knowledge Proofs]]
- [[Knowledge/AI Systems/AI Agent Security|AI Agent Security]]
- [[Knowledge/AI Systems/AI Agent Governance|AI Agent Governance]]
- [[Knowledge/AI Systems/AI Governance|AI Governance]]
- [[Knowledge/AI Systems/AI Conformity Assessment|AI Conformity Assessment]]
- [[Knowledge/Software Engineering/AI-Assisted Security Engineering|AI-Assisted Security Engineering]]
- [[Knowledge/Software Engineering/Software Supply Chain Security|Software Supply Chain Security]]

- [[Knowledge/Security/OpenID Connect|OpenID Connect]]

### 콘텐츠·지식

- [[Knowledge/AI Systems/AI Content Access|AI Content Access]]
- [[Knowledge/AI Systems/AI Content Monetization|AI Content Monetization]]

### 과학·건강

- [[Knowledge/AI Systems/AI for Scientific Discovery|AI for Scientific Discovery]]
- [[Knowledge/AI Systems/AI Medical Imaging|AI Medical Imaging]]
- [[Knowledge/AI Systems/AI Wellness Devices|AI Wellness Devices]]

## 개념 경계

| 분야 | 중심 질문 | 시작 문서 |
|---|---|---|
| 에이전트 | AI가 도구를 써서 일을 어떻게 끝내는가 | [[Knowledge/AI Systems/AI Agents|AI Agents]] |
| 평가 | 잘했는가와 무엇을 했는가를 어떻게 구분하는가 | [[Knowledge/AI Systems/Agent Evaluation|Agent Evaluation]], [[Knowledge/AI Systems/Agent Observability|Agent Observability]] |
| 보안·운영 | 어떻게 보호하고 누가 책임지는가 | [[Knowledge/AI Systems/AI Agent Security|AI Agent Security]], [[Knowledge/AI Systems/AI Agent Governance|AI Agent Governance]] |
| 개인정보 보호 암호 | 필요한 조건만 증명하면서 원본 비밀 노출을 어떻게 줄이는가 | [[Knowledge/Security/Zero-Knowledge Proofs|Zero-Knowledge Proofs]] |
| 조직 관리 | AI 전체를 어떻게 관리하고 요구사항을 판정하는가 | [[Knowledge/AI Systems/AI Governance|AI Governance]], [[Knowledge/AI Systems/AI Conformity Assessment|AI Conformity Assessment]] |
| 웹 콘텐츠 | 누가 읽고 어떻게 보상하는가 | [[Knowledge/AI Systems/AI Content Access|AI Content Access]], [[Knowledge/AI Systems/AI Content Monetization|AI Content Monetization]] |
| 건강 | 의료적 영상 해석과 생활습관 지원을 어떻게 구분하는가 | [[Knowledge/AI Systems/AI Medical Imaging|AI Medical Imaging]], [[Knowledge/AI Systems/AI Wellness Devices|AI Wellness Devices]] |
| 시계열 예측 | 여러 시계열에서 사전학습한 패턴을 새 예측에 어떻게 재사용하는가 | [[Knowledge/AI Systems/Time-Series Foundation Models|Time-Series Foundation Models]] |

## 읽는 순서

1. 관심 분야의 시작 문서에서 한 문장 정의와 범위를 확인합니다.
2. `혼동하기 쉬운 개념`에서 경계를 확인합니다.
3. `관련 개념`의 관계 라벨을 따라 다음 문서로 이동합니다.
4. 날짜별 사건과 원문은 해당 문서의 `최근 변화`와 데일리 매거진에서 확인합니다.

## 관련 인덱스

- [[Knowledge/AI Systems/AI Agent Security and Governance|AI Agent Security and Governance]]
- [[Knowledge/AI Systems/Agent Evaluation and Observability|Agent Evaluation and Observability]]
- [[Knowledge/AI Systems/AI Governance and Conformity Assessment|AI Governance and Conformity Assessment]]
- [[Knowledge/AI Systems/AI Content Access and Monetization|AI Content Access and Monetization]]
- [[Knowledge/AI Systems/AI Medical Imaging and Wellness Devices|AI Medical Imaging and Wellness Devices]]
- [[Knowledge Maps/AI Technology Knowledge Map|AI Technology Knowledge Map]]

## 출처

없음
