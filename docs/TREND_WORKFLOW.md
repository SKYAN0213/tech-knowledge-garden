...
2026-09-14부터 [분야·테마 조사 규칙](NEWS_THEMES.md)도 적용한다. 기술뿐 아니라 실적·자금·인력·생산의 변화와 이전 근거를 함께 검토한다. 기업별 사건은 일관된 기업명을 사용하고, 비교 수치·기간·계획/집행 구분·반대 근거·다음 확인을 남긴다. 월요일에는 기존 8시 실행 안에서 최근 일주일을 재검토한다. 테마나 기업명이 있다는 이유로 트렌드 주제·지식지도 노드를 만들거나 인과관계를 단정하지 않는다.
...
## 실제 회차 검토 기록 (2026-09-21)

이번 회차는 저장 후 `context`로 확인한 사건 ID `43924fdd4d64f158`를 같은 회차 Signals에 연결했다. CXMT 공정·제품 양산 발표의 비교 기준과 고객·출하 후속 질문을 company-cxmt에 기록했으며, 첫 관측으로 재사용 교훈을 만들지 않았다. 월요일 재검토는 최근 7일의 제품 공개·MOU·조직 변화·측정 조건을 구분하고 기존 반대 조건을 유지한다. 확인한 새 근거가 없는 주제는 문장이나 날짜를 갱신해 새 판단으로 꾸미지 않는다. 실제 검색·접근 실패·소급 후보 판정과 완료 상태는 비공개 Research 및 실행 영수증을 기준으로 한다.


## 2026-09-22 실제 정기 실행

- 기존 Drive 원본170개와 GitHub main을 대조한 뒤 신규9건을 작성했다. 8분야×2경로×국내외32행은 모두 부분 확인으로 기록했다. 등록48경로 접속과 원문 내용 검토를 구분했다.
- 원고 저장 후 context의 동일 회차 event_id로 Signals7개·6주제를 검토했다. 신규4주제와 기존2주제를 갱신하고 기존 관측·반대 조건을 보존했다. 심층은 순환에 따른 RamanOmics 논문 해설이며 새 교훈은 만들지 않았다.
- Drive176원본 fresh readback 후 단발 발행. 초기 RSS문구 검사 실패를 고친 뒤 테스트76개와 Actions35667412459 배포 성공, 공개13파일 및9기사 RSS·GitHub 분류·설명 전달을 확인했다. 상시 DRIVE_EXPORT_URL은 미설정이며 예약을 변경하지 않았다.
- 정규 발행 검증 후 9월3일 3사건의 공식 원문을 소급 재검토했다. 기존 event_id·회차 cutoff·RSS GUID를 유지한다. 배포 최종 영수증과 잔여량은 비공개 .local/retrospective/2026-09-22 및 CODEX_TASK_STATE.md에서 관리한다.
- 일반 문장의 후속 검증 설명도 RSS금지문구 검사와 충돌할 수 있어 게시 전 test:garden을 함께 실행한다. 검사 통과와 원문·배포 검증은 별도로 기록한다.

## 2026-09-23 실제 정기 실행

- 기존177 Drive 원본과 GitHub main을 대조하고 8분야 신규13건·연구 사업화 심층1건을 작성했다. 32개 지역/경로 행의 부분 확인과 접근 실패는 비공개 Research에 보존했다. 소급 후보의 날짜 경과를 제외 근거로 삼지 않았다.
- 원고 저장 후 context의 정확한 event_id로 Signals7개를 작성하고 신규 venture-envisagenics와 기존5주제의 판단을 검토했다. 계약 잠재 지급 총액과 현금 수령, 실제 도입과 제공 계획을 구분하며 새 재사용 교훈은 만들지 않았다.
- 변경8원본을 Drive에 저장하고 전체180원본을 새로 읽어 대조했다. 스냅샷 범위 식별자 누락을 수정한 뒤 Actions35797115362 성공과 공개17파일·13기사 RSS/GitHub 분류·설명을 확인했다. DRIVE_EXPORT_URL은 미설정이며 기존 연결을 통한 단발 발행이다.
- 정규 발행 후 9월2일4사건을 원문부터 소급 검토했다. 의료 정확도 평가는9개 전체가 아닌5개 데이터원임을 정정하고 기존 event_id·cutoff·RSS40개 GUID/pubDate를 유지했다. 최종 배포 및 잔여량은 .local/retrospective/2026-09-23과 실행 영수증으로 확인한다. 전체 소급 재검토 완료를 뜻하지 않는다.
