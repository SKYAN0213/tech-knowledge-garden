...
이 저장소의 `vault/`가 유일한 현재 Tech Knowledge다. 노트별 선별 없이 유형별 웹 화면을 만든다. 뉴스·브리핑을 기본 메뉴로 두고 개념과 지도는 맥락 링크로 제공한다. 보관용 원고·Archive·Trends·폴더 페이지는 웹 출력과 검색에서 제외한다. GPT는 조사·검증·집필을 맡고, 무료 로컬 스크립트는 기사 중복 연결, 헤드라인, 주간 관측과 RSS를 만든다. 별도 OpenAI API나 자동화 구독은 사용하지 않는다. Codex의 기존 ChatGPT 사용량 한도는 적용된다.
...
5. 일일 사건은 원고에, 재사용 정의는 기존 `vault/Knowledge/`의 canonical concept에 기록한다. 개념의 `최근 변화`에서 해당 기사·원문을 연결한다. 원고의 연결은 `[[Knowledge/AI Systems/AI Agents|AI Agents]]`처럼 vault 기준 경로를 사용한다. 새 개념을 만들었으면 사전 색인을 갱신한다. 모든 개념에는 concept_id(고정 ID), label(한글), group(의미 범주), keywords, verified_sources, last_reviewed를 작성한다. 관계는 relations 목록의 target(ID), type(uses/implements/evaluates/observes/controls/scope/contrast/informs), reason, basis(source/inference), evidence(URL 목록)로 기록한다. evidence URL은 해당 노트의 verified_sources에도 포함한다. 정의와 관계는 실제 읽은 일차 자료로 확인한다. Obsidian 지도와 웹 그래프는 이 메타데이터에서 함께 생성된다. 관련 개념에는 실제 연결과 이유만 적고, 비어 있는 상위·하위 분류나 중복된 연결 목록을 채워 넣지 않는다.
...
`npm run build`는 임시 폴더에 독자용 콘텐츠만 투영하고 Quartz로 Obsidian 문법을 변환한 뒤 독자 화면을 생성한다. `node scripts/verify-site.mjs`는 모든 출력 링크·문단 ID·검색 범위·그래프 목적지·RSS를 검사한다. 지도는 관계 기반 force layout과 라벨 충돌 해소를 사용한다. 원본 뉴스 이력은 날짜별 기록이며 개념 정의의 재검토와 구분한다. `scripts/rebuild-knowledge-20260913.mjs`는 완료한 일회성 이전이므로 예약 실행에서 재실행하지 않는다.
