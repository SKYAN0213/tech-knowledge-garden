...
5. 일일 사건은 원고에, 재사용 정의는 기존 `vault/Knowledge/`의 canonical concept에 기록한다. 개념의 `최근 변화`에서 해당 기사·원문을 연결한다. 원고의 연결은 `[[Knowledge/AI Systems/AI Agents|AI Agents]]`처럼 vault 기준 경로를 사용한다. 새 개념을 만들었으면 사전 색인을 갱신한다. 모든 개념에는 concept_id(고정 ID), label(한글), group(의미 범주), keywords, verified_sources, last_reviewed를 작성한다. 정의·키워드는 실제 읽은 일차 자료로 확인한다. 새 연결은 `connections` 목록에 `target`(기존 concept_id), `reason`(확인한 연관성)을 기록하고 원문 URL이 있으면 `evidence`를 추가한다. 방향·참조 유형·인용은 연결의 필수 조건이 아니다. 기존 `relations`의 유형·근거·source/inference 구분은 보존한다. 관련 개념 본문에는 `- [[Knowledge/…|개념]] — 확인한 이유`로 동일한 연결을 적는다. 빈 분류나 추측을 채워 넣지 않는다. Obsidian 지도는 확인한 개념 관계를, 웹 지도는 여기에 실제 키워드·뉴스 연결을 더해 생성한다. 규칙은 [연결 지도](CONNECTION_MAP.md)를 따른다.
...
`npm run build`는 임시 폴더에 독자용 콘텐츠만 투영하고 Quartz로 Obsidian 문법을 변환한 뒤 독자 화면을 생성한다. `node scripts/verify-site.mjs`는 모든 출력 링크·문단 ID·검색 범위·그래프 목적지·RSS를 검사한다. 지도는 Sigma.js의 WebGL과 ForceAtlas2 배치를 사용한다. 노드를 선택하면 최대 두 번의 연결로 닿는 관련 뉴스가 표시된다. 본문 키워드 연결에서 제목 외 섹션명·안내·출처 목록은 제외하며, 연결 이유와 경유 개념을 보존한다. 원본 뉴스 이력은 날짜별 기록이며 개념 정의의 재검토와 구분한다. `scripts/rebuild-knowledge-20260913.mjs`는 완료한 일회성 이전이므로 예약 실행에서 재실행하지 않는다.
