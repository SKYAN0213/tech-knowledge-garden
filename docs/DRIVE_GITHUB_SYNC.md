...
5. `Sync Drive and Publish Garden`을 수동 실행하고 배포 및 `https://skyan0213.github.io/tech-knowledge-garden/drive-sync.json`의 해시를 확인한다. 주소가 없으면 이 선택적 수동 실행은 오류로 종료한다. 8시 Codex 실행은 기존 Drive 연결을 사용한다.
...
## 매일 오전 8시에 새 브리핑을 작성할 때

`docs/SECTOR_BRIEFING.md`의 8개 분야 및 분야별 5건 기준을 적용한다.
...
- 상시 연결의 활성 여부는 저장소 변수 `DRIVE_EXPORT_URL`과 성공한 수동 동기화 실행을 함께 확인한다. 코드 설치나 초기 단발성 읽기는 상시 연결의 증거가 아니다.
