...
3. `python3 scripts/prepare-drive.py --collect`를 실행한다. 새 출처만 수집하고 이전 수집 결과는 보존한다. 재취재로 원문 변경을 수집할 때에는 기존 snapshot을 날짜별로 보존한 다음 새 버전을 저장한다. 차단·404·용량 초과·접근 실패를 숨기지 않는다. 현재 다운로드와 과거 취재 증거를 구분한다.
